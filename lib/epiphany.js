...
var Epiphany = function(options) {
	...
	this.routes = loaders.routes(this.items.routes, this.express, this.mw, this.config);

	this.templates = loaders.templates(this.items.templates);
...
Epiphany.prototype.preware = function(newPreware, method, reference) {
	ware.apply(this, [ 'preware' ].concat(arguments));
};

Epiphany.prototype.postware = function(newPreware, method, reference) {
	ware.apply(this, [ 'postware' ].concat(arguments));
};

function ware(type, newWare, method, ref) {
	var arr = this[type];
	if(method === 'before' || method === 'after') {
		var index = arr.indexOf(reference);
	} else 
		arr[method](newPreware);
}
...
Epiphany.prototype.module = function(module, last) {
	...
	// pick (order matters!)
	module = _.pick(module, [ 'models', 'mw', 'routes', 'navigation', 'templates' ]);
	...
	_.each(module, function(value, key) {
		...
		if(last)
			_app.items[key].push(value);
		else
			_app.items[key].splice(-1, 0, value);
	});
};
