...
var Epiphany = function(options) {
	...
	var configDirectories = [ __dirname + '/config' ];

	if(_.isString(options.config)) configDirectories.push(options.config);
	else if(!options.config) configDirectories.push(PWD + '/server/config');

	this.config = loaders.config(configDirectories);

	if(_.isObject(options.config)) this.config = _.merge(this.config, config);
...
Epiphany.prototype.init = function(options) {
	...
	this.mw = loaders.middleware(this.directories.mw, this.config);
