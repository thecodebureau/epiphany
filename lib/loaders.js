		...
		directories.forEach(function(directory) {
			function recurse( file) {
				...
				if(fs.statSync(file).isDirectory()) {
					...
					if(templates[name]) console.log(warningPrefix + 'Overriting template "' + name + '".');
				...
				if(key === 'redirects') {
					...
					var recurse = function(pages, tree) {
						return _.compact(_.map(pages, function(page) {
							...
							if(!page.name)
								if(page.title)
									page.name = page.title.toSpinalCase().replace(/[åä]/g, 'a').replace(/ö/g, 'o').toSpinalCase();
								else if (!page.path)
							...
							var pageClone = _.pick(page, [ 'name', 'pages', 'template', 'title', 'view' ]);
							...
							return page.nav === false ? undefined : _.pick(page, [ 'current', 'name', 'pages', 'path', 'title' ]);
						}));
					};
	...
	middleware: function(items, config) {
		function recurse(modules, obj) {
			...
			_.each(modules, function(module, key) {
				...
				if(obj[key]) console.log(warningPrefix + 'Overrwriting "' + key + '" middleware');
		...
		function recurse(routes, tree) {
			...
			routes.forEach(function(route) {
				...
				if(collection[path][method])
					console.log(warningPrefix + 'Overrwriting route: "' + method.toUpperCase() + ': ' + path + '.');
