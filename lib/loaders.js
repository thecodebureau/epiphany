...
module.exports = {
	config: function(configs) {
		...
		//		var cc = key.toCamelCase();

		//		if(key === 'globals') {
		//			module(global);
		//		} else if(_.isFunction(module)) {
		//			try {
		//				obj[cc] = module(configObject);
		//			} catch(e) {
		//				return;
		//			}
		//		} else if (_.isObject(module)) {
		//			if(module.defaults) {
		//				obj[cc] = _.merge(module.defaults, module[ENV]);
		//			} else {
		//				if(!obj[cc]) obj[cc] = {};
		//				recurse(module, obj[cc]);
		//			}
		//		} else {
		//			obj[cc] = module;
		//		}
		...
		function recurse(configs, obj) {
			configs = configs.filter(function(config) {
				return !_.isNull(config) && !_.isUndefined(config);
			});
			...
			while((length = keys.length) > 0) {
				keys.forEach(function(key, i) {
					...
					var halted = !configs.every(function(config) {
		...
		configs = configs.map(function(config) {
			function toCamelCase(obj) {
				_.each(obj, function(value, key) {
					...
					if(_.isObject(obj[cc]) && !_.isFunction(obj[cc]) && !_.isArray(obj[cc])) {
						toCamelCase(obj[cc]);
					}
			...
			if(!_.isObject(config)) config = requireDir(config, { recurse: true });
	...
	middleware: function(directories, config) {
		function recurse(modules, obj) {
			_.each(modules, function(module, name) {
				var cc = name.toCamelCase();

				if(_.isFunction(module)) {
					obj[cc] = module(config);

					if(cc !== name) delete obj[name];
				} else {
					if(!obj[cc]) obj[cc] = {};
	...
	routers: function(directories, mw, config) {
