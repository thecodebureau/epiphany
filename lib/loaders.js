...
module.exports = {
	...
	navigation: function(navigations, mw, epiphany) {
		var collection = {};
				...
				if(key !== 'redirects') {
					(function recurse(routes, tree) {
						...
						_.map(routes, function(route) {
							...
							if(route.path) {
								path = _.compact(route.path.split('/'));

								if(!/^\//.test(route.path)) {
									path = tree.concat(path);
								}
							} else {
								path = tree.concat(route.name);
							}
					...
					routes.forEach(function(arr) {
						epiphany.server.get(arr[0], function redirect(req, res) {
							res.redirect(arr[1]);
						});
					});
				}
