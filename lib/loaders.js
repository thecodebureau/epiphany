...
module.exports = {
	config: function(configs) {
		...
		while((length = keys.length) > 0) {
			keys.forEach(function(key, i) {
				...
				var halted = !configs.every(function propertyCollector(config) {
	...
	routers: function(directories, express, mw, config) {
		function recurse(obj, tree) {
			function init(prop, root) {
				var router = obj[prop](express, mw, config);
