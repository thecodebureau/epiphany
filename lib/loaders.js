...
module.exports = {
	config: function(directories) {
		...
		directories.forEach(function(directory) {
			var configModules = requireDir(directory);

			var left;

			while((left = _.keys(configModules).length) > 0) {
				recurse(configModules, configObject);

				if(left === _.keys(configModules.length)) {
					throw new Error('Circular reference in configuration');
				}
			}
		});
	...
	routers: function(directories, server, mw, config) {
