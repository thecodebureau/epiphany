...
module.exports = {
	...
	navigation: function(navigations) {
	...
	middleware: function(items, config) {
		function recurse(modules, obj) {
			_.each(modules, function(module, key) {
				...
				if(_.isFunction(module)) {
					...
					if(str.substr(str.indexOf('(') + 1,3) !== 'req')
		...
		items.forEach(function(item) {
			if(_.isString(item))
				item = _.mapKeys(requireDir(item, { recurse: true }), function(value, key) {
					return _.camelCase(key);
				});
	...
	routes: function(items, express, mw, config) {
				...
				if(_.isFunction(value)) {
					...
					routes.forEach(function(route) {
						function addRoute(str) {
							...
							if(!collection[p]) collection[p] = {};

							if(collection[p][method])
								console.log('WARNING!!! Overrwriting route: "' + method.toUpperCase() + ': ' + p + '.');

							collection[p][method] = mw;
						}
